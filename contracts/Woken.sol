...
contract Woken is Context, IERC20, Ownable {
    ...
    string private _name = "Woken";
    string private _symbol = "WOKEN";
    ...
    uint256 private numTokensSellToDoGood = 100_000 * 10**18;
    ...
    constructor () {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xE3F85aAd0c8DD7337427B9dF5d0fB741d65EEEB5);  // Volt V2 Router
    ...
    function _getValues(uint256 tAmount) private view returns (ValuesStruct memory) {
        ...
        RValuesStruct memory rvs = _getRValues(tAmount, tvs.tFee, tvs.tBurn, tvs.tMarketingAndTeam, tvs.tDoGood, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (TValuesStruct memory) {
        ...
        uint256 tDoGood = calculateDoGoodFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tBurn).sub(tMarketingAndTeam).sub(tDoGood);
        return TValuesStruct(tTransferAmount, tFee, tBurn, tMarketingAndTeam, tDoGood);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 tMarketingAndTeam, uint256 tDoGood, uint256 currentRate) private pure returns (RValuesStruct memory) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rMarketingAndTeam = tMarketingAndTeam.mul(currentRate);
        uint256 rDoGood = tDoGood.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rDoGood).sub(rBurn).sub(rMarketingAndTeam);
        return RValuesStruct(rAmount, rTransferAmount, rFee, rBurn, rMarketingAndTeam, rDoGood);
    }
    ...
    function _takeDoGoodFee(uint256 rDoGood, uint256 tDoGood) private {
        _rOwned[address(this)] = _rOwned[address(this)].add(rDoGood);
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tDoGood);
    }
    ...
    function calculateDoGoodFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_doGoodFee).div(
            10**2
        );
    }
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >= numTokensSellToDoGood;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
        ...
        //transfer amount, it will take tax, burn, doGood fee
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        _takeDoGoodFee(vs.rDoGood, vs.tDoGood);
